...
| 5 | „Ime fajla je hemijski metadata.“ | Filename je ljudska oznaka. `cu_n14_a.cif` nema Cu u formuli/atom sites; `Cu Kα` je izvor rendgenskog zračenja. | Element filter koristi strukturisana polja i cross-check, nikad substring imena. Fixture mora uhvatiti `cu` false positive. |
| 6 | „Formula u CIF-u je nepogrešiva.“ | Formula je prijavljena/izvedena stavka i može biti nesaglasna sa atom sites, occupancy, symmetry multiplicity ili \(Z\). | Ingest poredi formula sum/moiety, site composition, charge i density račun; konflikt ide u karantin. |
| 7 | „Formalni, oksidacioni i parcijalni naboj su isti broj.“ | Formal charge je Lewis knjigovodstvo; oxidation state jonska formalizacija; partial charge zavisi od modela i obično nije ceo broj. | Šema mora imati tri različita polja, metode i missing statuse. Ne kopirati jedno u drugo. |
...
| 14 | „Najveći fragment je uvek aktivni ligand/parent.“ | Counterion ili metalna mreža može biti veća; koordinacioni polimer nema jednostavan konačni „najveći molekul“. | Component role classification koristi hemiju i korisnički cilj. Nikad ne briši ostale komponente iz source of truth. |
...
| 27 | „Occupancy i multiplicity su isto.“ | Occupancy je populacija mesta; multiplicity broj symmetry-equivalent mesta. | Odvojena polja i testovi mass/formula round-trip-a. |
...
| 34 | „\(Z=Z'\).“ | \(Z\) broji formula units u ćeliji; \(Z'\) nezavisni ASU sadržaj u jednostavnom slučaju. Odnos uključuje multiplicity i može biti složen. | Nikad ne kopiraj jedno polje u drugo. Testiraj special positions i \(Z'>1\). |
...
| 68 | „Ekstrahovan PDF tekst je ono što korisnik vidi.“ | Embedded text layer može sadržati skriveni/stari sadržaj. U CCDC white paper-u strane 1–2 izvlače nevidljiv tekst o ultra-large GOLD docking-u. | Čuvaj raw extraction, render/OCR cross-check i QA status; konfliktni tekst ne ulazi u approved indeks. |
...
| 70 | „Skriveni PDF tekst je samo dokumentni problem, nema veze sa hemijom.“ | Pogrešan tekst može kontaminirati RAG, klasifikaciju teme, labels, deduplikaciju i provenance. | Dokumentni ingest koristi isti raw→validate→curate→derive obrazac kao CIF ingest. |
...
| 76 | „`not comparable` je score 0.“ | Nula tvrdi validno izmerenu odsutnost sličnosti; `not comparable` znači da merenje nije moguće/primenljivo. | Typed status i nullable score; UI ne boji missing ćeliju kao maksimalnu udaljenost. |
...
| 86 | „Ground truth je naziv query-ja ili database field.“ | Naziv izražava nameru; query constraints mogu biti slabiji. Kurirana polja mogu imati scope i missingness. | Label rubric, najmanje dva reviewera za critical set i adjudication sa evidence-om. |
...
| 90 | „Jedna ukupna metrika znači da sistem radi svuda.“ | Prosek može sakriti neuspeh za metale, solvates, disorder, velike/fleksibilne molekule ili missing 3D. | Predefinisani slices, confidence intervali i najgori relevantni slice ulaze u go/no-go. |
| 91 | „LLM može sam potvrditi hemijsku istinu iz teksta/CIF-a.“ | LLM može korisno objašnjavati, ali može halucinirati, pogrešno parsirati i nema eksperimentalni oracle. | Deterministički parser/validator + primarni izvori + ekspertni review; LLM output nije claim ledger source. |
...
| 96 | „CSD je akademska baza pa su raw podaci open.“ | CSD Portfolio i njegovi data files/subsets su licencirani proprietary sadržaj po javnim CCDC uslovima. | Raw CSD i masovni izvozi ostaju van repoa; konkretni institucijski ugovor je merodavan. |
...
| 98 | „Ako nešto mogu preuzeti preko API-ja, smem da ga keširam i delim.“ | Tehnički access, processing, retention, redistribution i publication su odvojena prava. | Permission matrix po operaciji, purpose i artefaktu; fail closed kada polje nedostaje. |
...
| 100 | „FAIR znači open, besplatno i bez logina.“ | FAIR Accessible može koristiti authentication/authorization; cilj je pronalaženje, standardan pristup, interoperabilnost i ponovna upotreba pod uslovima. | Restricted dataset može biti FAIR. UI prikazuje pristup/policy, ne lažno javni download. |
